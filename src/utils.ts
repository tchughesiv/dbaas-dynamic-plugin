...
export async function fetchInvAndConnNamespacesFromConfigs(installNS = '') {
  ...
  configs.forEach((config) => {
    let configSpec = { connectionNamespaces: [], disableProvisions: false }
    if (config.status?.conditions?.length > 0 && config.status.conditions[0].status === 'True') {
      configSpec = config.spec
      if (nsMap[config.metadata?.namespace]) {
        nsMap[config.metadata?.namespace].connectionNamespaces.push(...config.spec?.connectionNamespaces)
        nsMap[config.metadata?.namespace].disableProvisions = configSpec.disableProvisions
      } else {
        nsMap[config.metadata?.namespace] = configSpec
      }
